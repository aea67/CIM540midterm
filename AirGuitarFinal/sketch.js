...
var startMsg = "Listen, then play the notes in the order seen. " ;
...
function preload(){
  mySound = loadSound('assets/Tune2.mp3')
}
function setup()
{
  ...
  utry = createButton('YOU TRY');
  utry.position(411, 456);
  // utry.mousePressed(utry);
...
function drawSong1 (counter) {
  ...
  switch (counter) {
    ...
    case 1:
      ...
      console.log("end 1");          
          mySound.setVolume(0.7); //plays after hitting play button 2nd time. 
          mySound.play();
      break;
    case 2:
      console.log("start 2");
  ...
  console.log("end drawSong1"); // after every break it pops out this?
}

function playTune()
{
      console.log("playTune is playing")
...
function draw(){
  ...
  if(mouseX >= 290 && mouseX <= 335 && mouseY >= 370 && mouseY <= 376){
    ...
    if (counter <= 6 && tune1 == true) {
      drawSong1(counter); 
      counter++; // playTune(); //playtune multiple times at once
    }
    else if(counter = 7 && tune1 == true){counter = 0; frameRate(0.5);
      tune1 = false;}
  }
